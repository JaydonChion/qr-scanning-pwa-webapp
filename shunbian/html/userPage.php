...
<!-- 	style="width:100px; height:100px; margin-top:15px;"
 -->			<div>
			...
			<div id="selectDivision" class="centerdiv">
				...
				<div class="form-popup" id="myForm">
					<form action="/action_page.php" class="form-container">
						...
						<input
							type="number"
							placeholder="Enter Job Serial Number"
							name="vSerial"
							id="vSerialID"
							required
						/>
			...
			<ons-page id="deleteItem">
 			...
 			if(mysqli_num_rows($result) > 0 ){
						...
						while($row = mysqli_fetch_array($result)){   //Creates a loop to loop through results
								echo "<tr onclick='getData(".$row['No'].")'><td><b>". $row['Title'] . "</b></td><td>" . $row['Location'] . "</td><td>" . $row['Wage'] . "</td><td>" . $row['StartTime']. "</td></tr>";  
								}
  ...
  function getData(id){
		...
		var params = "id="+id;
         var xmlhttp = new XMLHttpRequest();
        xmlhttp.open("POST", "../phpscripts/getData.php",true);
      xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      xmlhttp.send(params);
            xmlhttp.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            if(this.responseText.length>0){
                ...
                alert("No additional info on this job");
            }
        }else{
